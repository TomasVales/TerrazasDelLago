...
const OrderAdmin = () => {
    ...
    const handleChangeStatus = async (order) => {
        ...
        if (status) {
            try {
                const res = await fetch(`${BACKEND_URL}/api/orders/${order.id}/status`, {
                    method: 'PUT',
                    headers: {
                ...
                await fetchOrders(); // Recarga los pedidos
        ...
        <div className="p-6">
            <div className="rounded-lg border border-gray-400 shadow-sm bg-white">
                <div className="flex flex-col space-y-1.5 p-6">
                    <h3 className="text-2xl pb-1 font-semibold leading-none tracking-tight">Pedidos</h3>
                        ...
                        <table className="w-full table-auto text-sm">
                            <thead className="border-b">
                                <tr className="border-b border-gray-200 bg-gray-100">
                                    ...
                                    <th className="p-4 text-left font-medium text-gray-500">Dirección</th>
                                    <th className="p-4 text-left font-medium text-gray-500">Productos</th>
                            ...
                            <tbody>
                                {orders.map((order) => (
                                    <tr key={order.id} className="border-b border-gray-200 hover:bg-gray-50">
                                        ...
                                        <td className={`p-4 capitalize ${order.status === 'completed' ? 'text-green-500' : order.status === 'canceled' ? 'text-red-500' : 'text-yellow-500'}`}>
                                            {order.status}
                                        </td>
                                        ...
                                        <td className="p-4 relative">
                                            <button onClick={() => setShowMenu(showMenu === order.id ? null : order.id)} className="cursor-pointer">
                                                <Ellipsis size={20} />
                                            </button>
                                            {showMenu === order.id && (
